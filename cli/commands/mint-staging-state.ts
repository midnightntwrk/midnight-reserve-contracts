...
import { getContractInstances, loadContractModule } from "../lib/contracts";
import { printSuccess, printError, writeTransactionFile } from "../utils";
...
export interface MintStagingStateOptions {
  network: string;
  output: string;
...
function getV2LogicScript(
  ...
  const module = loadContractModule(configSection, false);
...
export async function mintStagingState(
  options: MintStagingStateOptions,
): Promise<void> {
  const { network, output, validator, sign, outputFile, useBuild } = options;
  ...
  const v2LogicContract = getV2LogicScript(validator, network);
