...
/**
 * Maps deployment tx descriptions to the expected validator names.
 *
 * For non-threshold deployments: forever + two-stage are verifiable via minted NFT
 * policy IDs in outputs. The logic hash is verified separately through UpgradeState
 * datum checks (Check 3/4) since logic scripts are registered as stake credentials
 * and don't produce outputs with identifiable policy IDs.
 */
...
/**
 * Maps deployment tx descriptions to the validators whose hashes appear as
 * minted NFT policy IDs in the tx outputs. Logic scripts are excluded since
 * they are registered as stake credentials, not minted as NFTs.
 */
...
// Deployment txs that carry two-stage validators (and thus have main/staging NFT outputs)
...
function findValidatorByName(
  ...
  return validators.find((v) => {
    ...
    // title format: "file.validator_name.endpoint" or "file.validator_name"
    // Match on the validator_name segment (index 1)
...
/**
 * Extract unique policy IDs from Blockfrost output amounts.
 * Blockfrost represents native assets as "{policyId}{assetName}" concatenated hex strings.
 * Policy IDs are 56 hex chars (28 bytes). "lovelace" is the only non-asset unit.
 */
...
/**
 * Parse CBOR inline_datum hex string to extract UpgradeState fields.
 *
 * Blockfrost returns inline_datum as a CBOR hex string.
 * UpgradeState uses Aiken's @list annotation, so it encodes as a bare CBOR list:
 *   List [ByteString(logic), ByteString(""), ByteString(auth), ByteString(""), Int(round), Int(logic_round)]
 *
 * We also handle Constr(0, [...]) as a fallback in case the encoding differs.
 */
function parseUpgradeStateDatum(inlineDatumCbor: string): {
  ...
  try {
    ...
    // UpgradeState uses @list, so try List first, then Constr as fallback
    const items =
...
async function checkOnChainScriptHashes(
  ...
  // Check that we have exactly the 12 expected descriptions
  ...
  // Check each deployment tx by verifying NFT policy IDs in outputs
  for (const tx of deploymentTxs) {
    ...
    // Extract unique policy IDs from output assets (these are the minted NFT policy IDs = script hashes)
    const onChainPolicyIds = extractPolicyIds(utxos.outputs);

    const passed = sortedEqual(expectedPolicyHashes, onChainPolicyIds);

    // Also show which logic hash is expected (verified via datum in Check 3/4)
...
async function checkUpgradeStateDatums(
  ...
  for (const description of TWO_STAGE_DEPLOYMENT_DESCRIPTIONS) {
    ...
    // Find the output with the target NFT (main or staging)
...
export async function verify(options: VerifyOptions): Promise<void> {
  ...
  // Resolve Cardano network for Blockfrost
