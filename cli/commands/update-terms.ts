...
import type { UpdateTermsOptions } from "../lib/types";
...
export async function updateTerms(options: UpdateTermsOptions): Promise<void> {
  ...
  console.log(`\nUpdating Terms and Conditions on ${network} network`);
  ...
  const termsThresholdAddress = getCredentialAddress(
    network,
  ...
  const termsTwoStageAddress = getCredentialAddress(
    network,
  ...
  console.log("  Terms and conditions threshold:", termsThresholdUtxos.length);
  console.log("  Council forever:", councilForeverUtxos.length);
  console.log("  Tech auth forever:", techAuthForeverUtxos.length);
  console.log("  Terms and conditions two stage:", termsTwoStageUtxos.length);

  if (
    !termsForeverUtxos.length ||
    !termsThresholdUtxos.length ||
    !councilForeverUtxos.length ||
    !techAuthForeverUtxos.length ||
    !termsTwoStageUtxos.length
  ) {
    throw new Error("Missing required contract UTxOs");
  }

  const termsForeverUtxo = termsForeverUtxos[0];
  const termsThresholdUtxo = termsThresholdUtxos[0];
  const councilForeverUtxo = councilForeverUtxos[0];
  const techAuthForeverUtxo = techAuthForeverUtxos[0];
  const termsTwoStageUtxo = findUtxoWithMainAsset(termsTwoStageUtxos);
  ...
  const termsTwoStageDatum = termsTwoStageUtxo.output().datum();
  if (!termsTwoStageDatum?.asInlineData()) {
    throw new Error(
      "Missing inline datum on terms and conditions two-stage UTxO",
    );
  }

  const upgradeState = parse(
    Contracts.UpgradeState,
    termsTwoStageDatum.asInlineData()!,
  );
  ...
  const currentDatum = termsForeverUtxo.output().datum();
  if (!currentDatum?.asInlineData()) {
    throw new Error(
      "Missing inline datum on terms and conditions forever UTxO",
    );
  }

  console.log("  Has inline datum");
  const currentTermsState = parse(
  ...
  try {
    ...
    if (sign) {
      ...
      writeTransactionFile(
        ...
        tx.getId(),
        true,
        "Update Terms and Conditions Transaction",
      );
