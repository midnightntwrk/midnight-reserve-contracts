...
async function main() {
  ...
  const [currentThreshold, currentSignerMap] = currentCouncilState;
  ...
  try {
    ...
    const techAuthPrivateKeys = parsePrivateKeys("TECH_AUTH_PRIVATE_KEYS");
    console.log(
      `\n🔑 Signing with ${techAuthPrivateKeys.length} tech auth private keys...`,
    );
    ...
    for (let i = 0; i < techAuthPrivateKeys.length; i++) {
      const privateKeyHex = techAuthPrivateKeys[i];
      console.log(`  Signing with tech auth key ${i + 1}...`);
      ...
      try {
        ...
        console.error(`    ✗ Failed to sign with key ${i + 1}:`, err);
      }
    }
