...
import { addGlobalOptions, type GlobalOptions } from "../../lib/global-options";
...
import { getDatumHandler, extractLogicRound } from "../../lib/datum-versions";
...
const command: CommandModule<GlobalOptions, ChangeCouncilOptions> = {
  ...
  handler: async (argv) => {
    ...
    // Parse two-stage upgrade state to get logic hash and logic_round
    ...
    // Parse the full UpgradeState to get mitigation logic hash
    const fullUpgradeState = parseInlineDatum(
      councilTwoStageUtxo,
      Contracts.UpgradeState,
      parse,
    );
    const mitigationLogicHash = fullUpgradeState[1];
    ...
    const logicRound = extractLogicRound(foreverDatum);
    console.log("  Logic round:", logicRound);

    const handler = getDatumHandler("council", logicRound);
    const currentData = handler.decode(foreverDatum);
    ...
    const techAuthSigners = extractSignersFromCbor(
    ...
    const newCouncilForeverStateCbor = handler.encode(newData);
    ...
    const councilRequiredSigners = Number(
      (BigInt(currentCouncilSigners.length) * councilNum +
        (councilDenom - 1n)) /
        councilDenom,
    );
