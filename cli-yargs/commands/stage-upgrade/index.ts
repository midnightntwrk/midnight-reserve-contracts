...
export function builder(yargs: Argv<GlobalOptions>) {
   return yargs
      ...
      .option("sign", {
         type: "boolean",
         default: true,
         description: "Sign the transaction (requires TECH_AUTH_PRIVATE_KEYS and COUNCIL_PRIVATE_KEYS)",
      })
      .option("output-file", {
         type: "string",
...
export async function handler(argv: StageUpgradeOptions) {
   ...
   const techAuthRequiredSigners = thresholdToRequiredSigners(techAuthSigners.length, techAuthNum, techAuthDenom, "multisig threshold");
   const councilRequiredSigners = thresholdToRequiredSigners(councilSigners.length, councilNum, councilDenom, "multisig threshold");
