...
export function thresholdToRequiredSigners(
   ...
   if (!Number.isInteger(totalSigners) || totalSigners < 0) {
      throw new Error(`${context}: total signers must be a non-negative integer, got ${totalSigners}`);
   }

   if (denominator <= 0n) {
      throw new Error(`${context}: threshold denominator must be greater than zero, got ${denominator}`);
   }
