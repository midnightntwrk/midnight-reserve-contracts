...
Moved to [`docs/bridge/spec.md`](../bridge/spec.md). The bridge is being
aligned with the Committee Bridge Consensus MIP (`plans/bridge/`) and its
constraints are specified there, not here. `committee_bridge_forever` and
`committee_bridge_two_stage_upgrade` follow the shared FC/ILM/TS/TSM/TSS/TM/TSG/RUN
tags above with the `config.committee_bridge_*` parameters.
