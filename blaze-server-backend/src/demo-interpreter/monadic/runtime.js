...
class MonadicRuntime {
  ...
  async initialize() {
    try {
      const response = await this._fetch("/api/session/new", {
        method: "POST",
      });
      ...
      console.error(
        `[Runtime] Failed to connect to server at ${this.baseUrl}: ${error.message}`,
      );
      throw error;
    }
  }
  ...
  async getBalance(name) {
    const result = await this._fetch(
      `/api/wallet/${name}/balance?sessionId=${this.sessionId}`,
    );
  ...
  async getWalletUtxos(walletName) {
    const result = await this._fetch(
      `/api/wallet/${walletName}/utxos?sessionId=${this.sessionId}`,
    );
  ...
  async getContractState(contractAddress) {
    const result = await this._fetch(
      `/api/contract/${contractAddress}/utxos?sessionId=${this.sessionId}`,
    );
  ...
  async advanceTime(seconds) {
    const currentTimeResponse = await this._fetch(
      `/api/emulator/current-time?sessionId=${this.sessionId}`,
    );
  ...
  watchBalance(walletName, formatter = null) {
    const defaultFormatter = (balance) =>
      `${walletName}: ${(parseInt(balance || "0") / 1000000).toFixed(6)} ADA`;
    return this.watch(
      `Balance: ${walletName}`,
      { type: "balance", key: walletName },
  ...
  watchWalletUtxos(walletName, formatter = null) {
    const defaultFormatter = (utxos) => {
      const totalValue = (utxos || []).reduce(
        (sum, utxo) => sum + parseInt(utxo.amount || "0"),
        0,
      );
    ...
    return this.watch(
      `UTxOs: ${walletName}`,
      { type: "walletUtxos", key: walletName },
      formatter || defaultFormatter,
    );
  }
  ...
  watchContractState(address, formatter = null) {
    ...
    return this.watch(
      `State: ${address.substring(0, 20)}...`,
      { type: "contractState", key: address },
      formatter || defaultFormatter,
    );
  }
  ...
  async executeWatcher(watcher) {
    ...
    try {
      ...
      const dataChanged =
        canonicalStringify(watcher.lastRawData) !== canonicalStringify(rawData);
  ...
  async executeAllWatchers() {
    if (this.debug)
      console.log(`[Runtime] Executing all ${this.watchers.size} watchers...`);
    const promises = Array.from(this.watchers.values()).map((w) =>
      this.executeWatcher(w),
    );
  ...
  async _fetch(endpoint, options = {}) {
    ...
    if (!response.ok) {
      ...
      try {
        ...
        errorDetails =
          errorBody.error || errorBody.message || "No additional details.";
      } catch (e) {
...
class TransactionBuilder {
  ...
  addCollateral(utxos) {
    this._collateralUtxos = utxos.map((u) => ({
      txHash: u.txHash,
      outputIndex: u.outputIndex,
    }));
    return this;
  ...
  async submit() {
    if (this._operations.length === 0) {
      throw new Error(
        "Transaction has no operations. Add operations before submitting.",
      );
    }
    const body = {
    ...
    const result = await this._runtime._fetch(
      "/api/transaction/build-and-submit",
      {
        method: "POST",
        body: body,
      },
