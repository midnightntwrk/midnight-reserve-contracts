...
const {
  JavaScriptDemoExecutor,
} = require("../../../dist/demo-interpreter/core/JavaScriptDemoExecutor.js");
...
app.post("/demo/init", async (req, res) => {
  try {
    ...
    console.log(
      `[Demo Server] Creating new single session for demo: ${demo.name}`,
    );
    ...
    res
      .status(500)
      .json({ error: "Failed to initialize demo", message: error.message });
  }
});
...
const requireSession = (req, res, next) => {
  if (!currentSession) {
    return res
      .status(404)
      .json({ error: "No active session. Please initialize a demo first." });
  }
  next();
};
...
app.post("/demo/execute-block", requireSession, async (req, res) => {
  try {
    ...
    console.log(`[Demo Server] About to execute code block ${blockNumber - 1}`);
    const result = await currentSession.executor.executor.executeCodeBlock(
      blockNumber - 1,
    );

    currentSession.executedBlockIds.add(blockId);
    console.log(
      `[Demo Server] Block execution completed. Operation type: ${result.operationType}`,
    );
    ...
    console.error("[Demo Server] Block execution error caught:", error.message);
    console.error("[Demo Server] Full error stack:", error.stack);
    ...
    if (!res.headersSent) {
      ...
      console.error(
        "[Demo Server] Response already sent, cannot send error response",
      );
    }
  }
});
...
app.post("/demo/execute-watchers", requireSession, async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Failed to execute watchers", message: error.message });
  }
});
...
app.post("/demo/reset/:sessionId", async (req, res) => {
  try {
    ...
    const executor = new JavaScriptDemoExecutor(
      currentSession.demo,
      currentSession.executor.baseUrl,
    );
    await executor.initialize();
    ...
    res
      .status(500)
      .json({ error: "Failed to reset session", message: error.message });
  }
});
...
app.delete("/demo/session/:sessionId", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Failed to cleanup session", message: error.message });
  }
});
...
app.get("/api/demo-files", (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ success: false, error: "Failed to read demo files" });
  }
});
...
app.get("/demo-flows/:filename", (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ success: false, error: "Failed to serve demo file" });
  }
});
