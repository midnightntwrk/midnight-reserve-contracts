...
class VariableResolver {
    ...
    resolveString(template) {
        ...
        resolved = resolved.replace(expressionRegex, (match, expression) => {
            ...
            try {
                ...
                throw new Error(`Failed to resolve expression '${expression}': ${error.message}`);
            }
        });
    ...
    callBuiltInFunction(expression) {
        ...
        switch (functionName) {
            ...
            case 'multiply':
                if (args.length !== 2)
                    throw new Error('multiply() requires exactly 2 arguments');
                const m1 = typeof args[0] === 'number' ? args[0] : Number(args[0]);
                const m2 = typeof args[1] === 'number' ? args[1] : Number(args[1]);
                return m1 * m2;
            case 'formatAda':
                if (args.length !== 1)
                    throw new Error('formatAda() requires exactly 1 argument');
                const lovelace = typeof args[0] === 'number' ? args[0] : Number(args[0]);
                return `${(lovelace / 1000000).toFixed(6)} ADA`;
            case 'formatUnixTime':
                if (args.length !== 1)
                    throw new Error('formatUnixTime() requires exactly 1 argument');
                const timestamp = Number(args[0]);
                return new Date(timestamp * 1000).toISOString();
            case 'computeContractAddress':
                if (args.length !== 1)
                    throw new Error('computeContractAddress() requires exactly 1 argument');
                const { contractAddress } = (0, script_utils_1.computeScriptInfo)(String(args[0]));
                return contractAddress;
            case 'computeScriptHash':
                if (args.length !== 1)
                    throw new Error('computeScriptHash() requires exactly 1 argument');
                const { scriptHash } = (0, script_utils_1.computeScriptInfo)(String(args[0]));
                return scriptHash;
            default:
