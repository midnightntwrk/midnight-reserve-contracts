...
table). Node kind from the NFT name. `Register` links one key per anchor
input (limitation kept for simplicity; spec §4.4). This is what makes account and batcher
...
- register: ok after head; ok between two nodes; ok at tail; wrong order
  ...
  weights below/above 1000, zero weight, no destinations.
